...
export default function AddDishPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim()) {
      setError("Kailangan ang pangalan ng ulam.");
      return;
    }
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--color-surface)" }}>
      ...
      >
        ...
        <h1 className="font-bbt text-xl tracking-wider">MAG-ADD NG ULAM</h1>
      </header>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl mx-auto px-6 py-8 flex flex-col gap-6">
              ...
              <div key={idx} className="flex items-center gap-2">
                ...
                {ingredients.length > 1 && (
                  <button
                    ...
                    className="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-opacity hover:opacity-70"
        ...
        <button
          ...
          style={{ background: "var(--color-primary)", color: "var(--color-on-primary)" }}
        >
          {submitting ? "SINE-SAVE..." : "I-SAVE ANG ULAM"}
        </button>
      </form>
    </div>
  );
